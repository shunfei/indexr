...
public class TaskMemoryManager {
     ...
     */
    final MemoryMode tungstenMemoryMode = MemoryMode.OFF_HEAP;
    ...
    public long acquireExecutionMemory(
        ...
        synchronized (this) {
            long got = memoryManager.acquireMemory(required, taskAttemptId);
                    ...
                    if (c != consumer && c.getUsed() > 0) {
                        try {
                            ...
                            if (released > 0 && mode == tungstenMemoryMode) {
                                ...
                                got += memoryManager.acquireMemory(required - got, taskAttemptId);
                                if (got >= required) {
                                    break;
                                }
                            }
