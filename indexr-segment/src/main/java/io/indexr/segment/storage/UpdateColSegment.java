...
public class UpdateColSegment {
    ...
    private static StorageColumn getStorageColumn(StorageSegment baseSegment,
        ...
        DPColumn dpColumn = new DPColumn(baseSegment.version(), baseSegment.mode(), newColId, newColumn.name, newColumn.sqlType, newColumn.isIndexed, 0, storePath);
        dpColumn.initUpdate();
    ...
    public static StorageSegment newColumnSchema(String name,
        ...
        SegmentSchema targetSchema = new SegmentSchema(
                targetColumnSchema.stream().map(c -> new ColumnSchema(c.name, c.sqlType, c.isIndexed)).collect(Collectors.toList()));
    ...
    public static StorageSegment addColumn(String name,
        ...
        for (ColumnSchema cs : baseSegment.schema().getColumns()) {
            targetColumns.add(new UpdateColSchema(cs.getName(), cs.getSqlType(), cs.isIndexed(), cs.getName()));
        }
