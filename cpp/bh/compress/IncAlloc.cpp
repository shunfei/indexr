...
const double IncAlloc::GROWSIZE = 1.2;
...
IncAlloc::IncAlloc(uint fsize)
{
	...
	used = 0;
	firstsize = fsize;
	frags = new std::vector<void*>[MAXFRAGSIZE+1];
}
...
void* IncAlloc::_alloc_search(uint size)
{
	...
	BHASSERT_WITH_NO_PERFORMANCE_IMPACT(blk == blocks.size());
	uint bsize = firstsize;
	if(blk > 0) {
		bsize = (uint)(blocks[blk-1].size * GROWSIZE + ROUNDUP) / ROUNDUP * ROUNDUP;
		BHASSERT_WITH_NO_PERFORMANCE_IMPACT(bsize > blocks[blk-1].size);
...
{
	memblock = 0; memalloc = 0; uint memfree = 0;
	uint i;
